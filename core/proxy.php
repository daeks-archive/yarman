  ...
  switch (network::get('action')) {
    case 'render':
      if (network::get('emulator') != '' && network::get('id') != '') {
        ...
        $obj = db::instance()->read('fields', network::get('type'), 'path').DIRECTORY_SEPARATOR.network::get('emulator').DIRECTORY_SEPARATOR.$id;
    ...
    case 'upload':
      if (network::get('emulator') != '' && network::get('type') != '' && isset($_FILES['object']) && $_FILES['object']['error'][0] == 0) {
        ...
        if (move_uploaded_file($_FILES['object']['tmp_name'][0], db::instance()->read('fields', network::get('type'), 'path').DIRECTORY_SEPARATOR.network::get('emulator').DIRECTORY_SEPARATOR.$output)) {
            network::success(db::instance()->read('fields', network::get('type'), 'path').DIRECTORY_SEPARATOR.network::get('emulator').DIRECTORY_SEPARATOR.$output);
        } else {
