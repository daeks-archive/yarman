...
function enable_yarman() {
    ...
    local old_config="$(grep "yarman\.sh" /etc/rc.local)"

    if _is_enabled_yarman; then
        if [[ "$config" == "$old_config" ]]; then
            printMsgs "dialog" "Nothing done. yarman is already enabled in /etc/rc.local."
            return
        else
            dialog --yesno "yarman is already enabled in /etc/rc.local with the following config:\n\n$old_config\n\nWe are about to update it to:\n\n$config\n\nDo you want to proceed?" 22 76 2>&1 >/dev/tty \
            || return
        fi
    fi
